    ...
    internal class SendGridService
    {
        ...
        public async Task SendBatch(SqlServer emailServer, string groupCode, string attachmentFolder, string smfAutoBatchEmailStartDate, int smfAutobatchEmailMaxAttempt, string fromEmail)
        {
            try
            {
                ...
                DataTable emails = await emailServer.GetData("SELECT * FROM dEmails LEFT JOIN dEmailsLinked on emkEmailID = emgMessageID and emkType = 'JOB' WHERE emgFunction = 'S' and emgSentOK = 0 and emgAddedDate >= @smfAutoBatchEmailStartDate and emgSendAttemptCount < @smfAutobatchEmailMaxAttempt and emgSystem = @groupCode", parameters);
